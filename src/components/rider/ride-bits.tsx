  ...
  return (
    <motion.button
      ...
      className={cn(
        ...
        selected ? "border-ink bg-white shadow-card" : "border-transparent hover:border-zinc-300",
      )}
    >
      <Image src={VEHICLE_IMAGES[vt.code]} alt={vt.name} width={96} height={64} className={cn("h-14 w-24 shrink-0 object-contain mix-blend-multiply transition-transform", selected && "scale-105")} />
      <span className="min-w-0 flex-1">
        <span className="flex items-center gap-2">
          <span className="text-[15px] font-semibold leading-tight">{vt.name}</span>
