...
const avatarTones = ["bg-brand-400 text-ink", "bg-blue-500 text-white", "bg-violet-500 text-white", "bg-rose-500 text-white", "bg-amber-400 text-ink", "bg-teal-500 text-white", "bg-ink text-white"];
...
export function Avatar({ name, src, size = "md", className, tone }: { name: string; src?: string | null; size?: "xs" | "sm" | "md" | "lg" | "xl"; className?: string; tone?: string }) {
  ...
  return (
    <span className={cn("inline-flex shrink-0 select-none items-center justify-center overflow-hidden rounded-full font-semibold", dims, tone ?? auto, className)} aria-hidden>
      {src ? (
...
export function Badge({ tone = "neutral", children, className, dot }: { tone?: Tone; children: React.ReactNode; className?: string; dot?: boolean }) {
  return (
    <span className={cn("inline-flex items-center gap-1.5 whitespace-nowrap rounded-full px-2.5 py-1 text-[11px] font-semibold leading-none", toneClasses[tone], className)}>
      {dot && <span className={cn("h-1.5 w-1.5 rounded-full", tone === "success" || tone === "brand" ? "bg-brand-500 animate-pulse-dot" : "bg-current opacity-70")} />}
      {children}
    </span>
  );
}
...
export function Card({ className, children, padded = true, ...props }: React.HTMLAttributes<HTMLDivElement> & { padded?: boolean }) {
  return (
    <div className={cn("rounded-xl border border-zinc-200/80 bg-white shadow-card", padded && "p-4", className)} {...props}>
      {children}
    </div>
  );
}
  ...
  return (
    <div className={cn("flex flex-col gap-1 rounded-xl p-4", tones, className)}>
      <div className="flex items-center justify-between gap-2">
        <p className={cn("text-[11px] font-semibold uppercase tracking-wide", tone === "light" ? "text-muted" : "opacity-75")}>{label}</p>
        {icon && <span className="opacity-80">{icon}</span>}
      </div>
      <p className="text-2xl font-bold leading-none tracking-tight">{value}</p>
      {(sub || delta) && (
        <p className={cn("text-[11px]", tone === "light" ? "text-muted" : "opacity-75")}>
          {delta && <span className={cn("mr-1 font-semibold", delta.up === false ? "text-danger" : "text-brand-600")}>{delta.value}</span>}
          {sub}
        </p>
      )}
    </div>
...
export function EmptyState({ icon, title, description, action, className, compact }: { icon?: React.ReactNode; title: string; description?: string; action?: React.ReactNode; className?: string; compact?: boolean }) {
  return (
    <div className={cn("flex flex-col items-center justify-center text-center", compact ? "gap-2 py-6" : "gap-3 py-12", className)}>
      {icon && <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-surface-2 text-zinc-500">{icon}</div>}
      <div>
        <p className="text-sm font-semibold">{title}</p>
        {description && <p className="mx-auto mt-1 max-w-xs text-xs font-normal text-muted text-balance">{description}</p>}
      </div>
...
export function Divider({ className, label }: { className?: string; label?: string }) {
  if (label)
    return (
      <div className={cn("flex items-center gap-3 text-[11px] font-medium uppercase tracking-wide text-zinc-400", className)}>
...
/* ------------------------------------------------------------------ */
/* List row (sidebar items, settings)                                   */
/* ------------------------------------------------------------------ */

export function ListRow({ icon, title, description, href, onClick, right, danger, className, badge }: { icon?: React.ReactNode; title: React.ReactNode; description?: React.ReactNode; href?: string; onClick?: () => void; right?: React.ReactNode; danger?: boolean; className?: string; badge?: React.ReactNode }) {
  const inner = (
    <>
      {icon && <span className={cn("flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-surface-2", danger ? "text-danger" : "text-ink")}>{icon}</span>}
      <span className="min-w-0 flex-1">
        <span className={cn("block truncate text-sm font-semibold", danger && "text-danger")}>{title}</span>
        {description && <span className="block truncate text-xs font-normal text-muted">{description}</span>}
      </span>
      {badge}
      {right ?? ((href || onClick) && <ChevronRight size={18} className="text-zinc-400" />)}
    </>
  );
  const cls = cn("flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left transition-colors hover:bg-surface-2 active:bg-surface-3", className);
  if (href) return <Link href={href} className={cls}>{inner}</Link>;
  if (onClick) return <button type="button" onClick={onClick} className={cls}>{inner}</button>;
  return <div className={cls.replace("hover:bg-surface-2 active:bg-surface-3", "")}>{inner}</div>;
}
...
export function Segmented<T extends string>({ value, onChange, options, className, size = "md" }: { value: T; onChange: (v: T) => void; options: { value: T; label: React.ReactNode }[]; className?: string; size?: "sm" | "md" }) {
  return (
    <div className={cn("inline-flex w-full rounded-xl bg-surface-2 p-1", className)} role="tablist">
      {options.map((o) => {
        ...
        return (
          <button
            ...
            className={cn(
              ...
              active ? "bg-white text-ink shadow-card" : "text-zinc-500 hover:text-ink",
            )}
          >
            {o.label}
          </button>
...
export function TopBar({ title, subtitle, back, right, className, transparent, onBack }: { title?: React.ReactNode; subtitle?: React.ReactNode; back?: boolean | string; right?: React.ReactNode; className?: string; transparent?: boolean; onBack?: () => void }) {
  ...
  /* Inside a desktop shell the page title belongs in the shell's own header. */
  React.useEffect(() => {
    if (!inShell) return;
    setHeader({
  ...
  return (
    <header className={cn("z-20 flex h-14 shrink-0 items-center gap-2 px-3", transparent ? "absolute inset-x-0 top-0" : "bg-white", className)}>
      {back && (
        <button type="button" onClick={goBack} aria-label="Go back" className={cn("flex h-10 w-10 items-center justify-center rounded-full", transparent ? "bg-white shadow-card" : "hover:bg-surface-2")}>
