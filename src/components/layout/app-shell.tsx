...
export function AppShell({
  ...
  const activeTone = tone ?? (user.role?.toLowerCase() as "rider" | "driver") ?? "rider";
  const items = activeTone === "rider" ? RIDER_NAV : DRIVER_NAV;
  const home = activeTone === "rider" ? ROUTES.rider.home : ROUTES.driver.home;
  const accent = activeTone === "driver" ? "text-driver-400" : "text-brand-300";
    ...
    <nav className="flex flex-1 flex-col gap-1 overflow-y-auto px-3">
      {items.map((item) => {
        ...
        return (
          ...
          >
            {active && <span className={cn("absolute inset-y-2 left-0 w-1 rounded-r-full", activeTone === "driver" ? "bg-driver-500" : "bg-brand-400")} />}
  ...
  const account = (
    <div className="mt-auto px-3 pt-3">
      <div className="flex items-center gap-3 rounded-xl bg-white/5 p-3">
        <Avatar name={user.name} src={user.profilePhotoUrl} size="sm" tone={activeTone === "driver" ? "bg-driver-500 text-white" : "bg-brand-400 text-ink"} />
  ...
  const brand = (
    <div className="mb-6 flex items-center justify-between px-6">
      ...
      <span className="rounded-md bg-white/10 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-white/70">{activeTone}</span>
    </div>
  );
    ...
    <div className="flex h-dvh w-full overflow-hidden bg-zinc-50 text-ink">
          ...
          <>
            ...
            >
              ...
              <div className="flex items-center gap-3 px-5 pb-4">
                <Avatar name={user.name} src={user.profilePhotoUrl} size="md" tone={activeTone === "driver" ? "bg-driver-500 text-white" : "bg-brand-400 text-ink"} />
      ...
      <div className="relative flex min-w-0 flex-1 flex-col">
        <header className="flex h-14 shrink-0 items-center gap-2 border-b border-zinc-200 bg-white px-3 lg:px-6">
          ...
          <Link href={activeTone === "rider" ? ROUTES.rider.profile : ROUTES.driver.profile} className="hidden items-center gap-2 rounded-lg px-1.5 py-1 hover:bg-surface-2 sm:flex">
            <Avatar name={user.name} src={user.profilePhotoUrl} size="xs" tone={activeTone === "driver" ? "bg-driver-500 text-white" : "bg-navy-900 text-white"} />
